...
void _AdiabaticHeatingTermPpc_Init( void* _self, int temperatureLabel, int velocity_id, PpcManager* mgr, int cpLabel, int alphaName, int external_id, int pressure_id ) {
	AdiabaticHeatingTermPpc* self = (AdiabaticHeatingTermPpc*)_self;
...
void _AdiabaticHeatingTermPpc_AssignFromXML( void* _self, Stg_ComponentFactory* cf, void* data ) {
    ...
    pres = PpcManager_GetField( mgr, cf, (Stg_Component*)self, "PressureField", True );
	_AdiabaticHeatingTermPpc_Init(
		...
		PpcManager_GetPpcFromDict( mgr, cf, self->name, (Dictionary_Entry_Key)"ExternalFunction", "ExternalFunction" ),
		pres
		 );
}
...
void _AdiabaticHeatingTermPpc_AssembleElement( void* _self, ForceVector* forceVector, Element_LocalIndex lElement_I, double* elForceVec ) {
  ...
  for ( cParticle_I = 0 ; cParticle_I < cellParticleCount ; cParticle_I++ ) {
	 ...
	 /* get pressure */
	 err = PpcManager_Get( self->manager, lElement_I, particle, self->pressure_id, &pressure );
	 assert( !err );
	 ...
	 err = PpcManager_Get( self->manager, lElement_I, particle, self->external_id, &external );
	 assert( !err );
	...
	if (external == 1){
		...
		alphaT = ((pow(temperature,4)*p1+pow(temperature,3)*p2+pow(temperature,2)*p3+pow(temperature,1)*p4+p5)/((Palpha/1e9)*facP+1.))*1e-5;
		adiabaticHeating = -1.0 * temperature * alphaT * verticalVelocity * gravityNorm / cp;
	}
	else {
